...
**Non-markdown file items** — Files synced from Canvas as binary items (`.svg`,
`.pdf`, etc.) live in `course/` but are not `.md` files. Docusaurus's docs
plugin only processes markdown by default, so these files are silently ignored
and never appear in the sidebar. No explicit `exclude` configuration is needed.
