...
async function canvasRequest(method, path, body) {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    if (attempt > 0) {
      ...
      log.info(
        `[canvas] Retrying ${method} ${path} (attempt ${attempt + 1}/${MAX_RETRIES + 1}) after ${delay}ms...`,
      );
      await sleep(delay);
    }
    ...
    try {
      ...
      // Network error (ECONNRESET, timeout, etc.). A dropped connection
      // leaves it unknown whether the server processed the request, so
      // non-idempotent POSTs are not retried — a duplicate page or
      // assignment is worse than a clean failure.
      if (method !== 'POST' && attempt < MAX_RETRIES) continue;
      throw new Error(
        `Canvas API ${method} ${path} failed after ${attempt + 1} attempt(s): ${err.message}`,
      );
    }
    ...
    while (nextUrl) {
      ...
      for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        ...
        try {
          ...
          throw new Error(
            `Canvas API pagination GET ${nextUrl} failed after ${MAX_RETRIES + 1} attempts: ${err.message}`,
          );
        }

        const nextRemaining = nextResponse.headers.get(
          'x-rate-limit-remaining',
        );
        if (
          nextRemaining !== null &&
          Number(nextRemaining) < RATE_LIMIT_THRESHOLD
        ) {
          await sleep(RATE_LIMIT_DELAY_MS);
        }

        if (!nextResponse.ok) {
          if (isRetryable(nextResponse.status) && attempt < MAX_RETRIES)
            continue;
          let errBody;
          try {
