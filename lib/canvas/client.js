...
async function canvasRequest(method, path, body) {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    if (attempt > 0) {
      ...
      log.info(`[canvas] Retrying ${method} ${path} (attempt ${attempt + 1}/${MAX_RETRIES + 1}) after ${delay}ms...`);
      await sleep(delay);
    }
    ...
    try {
      ...
      throw new Error(`Canvas API ${method} ${path} failed after ${attempt + 1} attempt(s): ${err.message}`);
    }
