...
/**
 * Alert styling: GFM type -> color, icon filename. Titles come from the
 * course language (lib/config/labels.js) via options.alertTitles.
 */
const ALERT_CONFIG = {
  note:      { color: '#4bafe1', icon: 'info.svg' },
  tip:       { color: '#64c8c8', icon: 'tip.svg' },
  important: { color: '#967dc8', icon: 'important.svg' },
  warning:   { color: '#ffc87d', icon: 'warning.svg' },
  caution:   { color: '#fa6432', icon: 'caution.svg' },
  check:     { color: '#00283c', icon: 'check.svg' },
};
...
function markdownToHtml(markdownContent, options = {}) {
  ...
  marked.use(markedAlert({
    variants: [
      { type: 'check', icon: '', title: alertTitles.check },
    ],
  }));
