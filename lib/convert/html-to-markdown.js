...
function buildFrontmatter(item, canvasType, existingFrontmatter) {
  ...
  if (canvasType === 'page') {
    ...
    // A module item names the quiz it links in content_id, a Quiz object names
    // itself in id. Both shapes reach this, depending on the caller.
    data.canvas_id =
      item.content_id != null ? item.content_id : item.id || null;
