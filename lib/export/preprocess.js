...
const { maskCodeRegions } = require('../convert/link-resolver');

/**
 * GFM alert kind -> canonical export kind. `attention` is an alias for
 * `caution`; `check` is a project-specific extra. Mirrors the handling in
...
function rewriteImagePaths(md, relativePath, courseDir) {
  ...
  return replaceOutsideCode(md, /!\[[^\]]*\]\(([^)]+)\)/g, (real) => {
    ...
    if (/^(https?:\/\/|\/\/|#|mailto:|data:)/.test(bare) || bare.startsWith('/')) {
      return real;
    }
...
function rewriteCrossLinks(md, relativePath, ctx) {
  ...
  return replaceOutsideCode(md, /\[([^\]]+)\]\(([^)]+)\)/g, (real) => {
    ...
    if (linkMap && courseId != null) {
      ...
      if (entry) {
        const url =
          entry.canvasType === 'assignment'
            ? `/courses/${courseId}/assignments/${entry.canvasId}`
            : `/courses/${courseId}/pages/${entry.canvasId}`;
        return `${text}^[Online: ${url}]`;
      }
    }
    return text;
  });
