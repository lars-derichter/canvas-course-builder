 ...
 * @param {string} [opts.title] - Title for the YAML frontmatter.
 * @param {string} [opts.subtitle] - Subtitle for the YAML frontmatter.
 * @returns {string}
 */
function generateToc(modules, opts = {}) {
  const lines = ['---', `title: "${(opts.title || 'Cursus').replace(/"/g, '\\"')}"`];
  if (opts.subtitle) lines.push(`subtitle: "${opts.subtitle.replace(/"/g, '\\"')}"`);
  lines.push('---', '');
  ...
  for (const mod of modules) {
    const items = flattenItems(mod.items).filter(
      (n) => n.type === 'item' && (!opts.flagged || (n.frontmatter && n.frontmatter.export === true)),
    );
