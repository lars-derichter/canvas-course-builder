 ...
 *  in lib/canvas/icons.js and the alert kinds in export-styles/filter.lua. */
...
function parseTokens(css) {
  ...
  const tokens = {};
  const declaration = new RegExp(`${TOKEN_PREFIX}([a-z0-9-]+)\\s*:\\s*([^;]+);`, 'gi');
  let match;
...
function loadTheme(rootDir) {
  const root = path.resolve(rootDir || require('../../cli/project-root').PROJECT_ROOT);
  if (cache.has(root)) return cache.get(root);

  const requested = String(loadCourseConfig(root).theme || DEFAULT_THEME).trim();
  let name = requested;
...
function themeFingerprint(theme) {
  const canonical = Object.entries(theme.colors)
    ...
    .join(';');
  return require('crypto').createHash('sha1').update(canonical).digest('hex').slice(0, 12);
}
