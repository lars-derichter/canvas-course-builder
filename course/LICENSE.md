---
unlisted: true
---

# Course content licence

The course materials in this folder are licensed under the
[Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International licence (CC BY-NC-SA 4.0)](https://creativecommons.org/licenses/by-nc-sa/4.0/),
unless this file says otherwise.

This file belongs to the course author. If you created your course from the
Canvas Course Builder template, the content of `course/` is yours: keep this
licence, adapt it, or replace it with one of your own choosing. The
`docs/customization.md` guide explains where licensing is declared.

The Canvas Course Builder tooling itself is MIT licensed (see the `LICENSE` file
in the repository root). Third-party assets bundled with the template are listed
in `THIRD-PARTY.md` and are covered by neither licence.
