...
An IMS CP 1.1 manifest with one `<resource>` of `type="imsqti_xmlv1p2"`
whose `href` and `<file>` point at `<ident>/<ident>.xml`.

## Assessment XML

`<questestinterop>` → `<assessment title="…">` → one `<section>` → one
`<item>` per question. Per item:

- `<qtimetadata>` with fields `question_type` (see the table in SKILL.md)
  and `points_possible`.
...
- `<resprocessing>` with an `<outcomes>` `SCORE` decvar (maxvalue 100) and
  `<respcondition>`s that `<setvar>` SCORE to 100 for the correct response.
  For multiple answers, one `<and>` condition requiring all correct choices
  and `<not>` on the others. Essay items get no scoring condition.
