...
- `.canvas-sync.json` — Schema version 2. Tracks module IDs, per-item IDs (`canvas_id`, `canvas_type`), `last_sync` timestamp, `icons` (alert icon file IDs), and `files` (embedded file Canvas IDs and URLs). Gitignored, generated by CLI. Saved incrementally after each module during push/pull.
